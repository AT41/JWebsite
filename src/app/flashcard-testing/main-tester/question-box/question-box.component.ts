...
export class QuestionBoxComponent {
	...
	@Input() options?: QuestionBoxOptions = {markQuestionsImmediately: true, showAnswersImmediately: true};
	...
	private waitAfterSubmit = 1000;
  	...
  	constructor(private statService: StatService) { 
		...
		this.formControl.valueChanges.subscribe(userAnswer => 
			this.userAnswerChange.emit(userAnswer)
		);
	}
	...
	onNext() {
  	...
  	private submit() {
		...
		this.statService.incrementStats(this.mainTesterCard.answer === this.formControl.value,  this.mainTesterCard.cardId).subscribe();
		setTimeout(() => {
